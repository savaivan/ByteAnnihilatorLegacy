...
if (isMainThread) {
    ...
    console.log(`ВСЕ ФАЙЛИКИ - ${filesList.length}`);
    ...
    console.log(`ФАЙЛИКИ МЕЧЕНЫЕ - ${MECHENIE_filesList.length}`);
    let partsize = Math.ceil(MECHENIE_filesList.length / config.workers);
    console.log(`ФАЙЛИКИ МЕЧЕНЫЕ ЧАСТИ - ${partsize}`);

    for (let i = 0; i < config.workers; i++) {
        let deathchunk = MECHENIE_filesList.slice(i, i + partsize);
        const worker = new Worker(__filename, {workerData: deathchunk});
        worker.on("message", msg => console.log(`РАБ ${worker.threadId} СКАЗАЛ: ${msg}`));
        worker.on("error", err => console.error(err));
        worker.on("exit", code => {if (code != 0) {console.log(`РАБ ЗАКОНЧИЛ С КОДОМ ${code}.`)}});
    }
}

else {
    const data = workerData;
    parentPort.postMessage(`ПОЛУЧИЛ СПИСОЧЕК РАЗМЕРОМ ${data.length}`);
    ...
    parentPort.postMessage(`ЗАКОНЧИЛ!!!`);
}
